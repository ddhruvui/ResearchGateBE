...
function token () {
  // Strip wrapping quotes the way lib/db.js does for MONGO_URI. A value pasted
  // into a dashboard env var as "abc" reaches us with the quotes attached, and
  // EODHD answers that with a 401 that reads exactly like a revoked key.
  const t = (process.env.EODHD_API_TOKEN || '').trim().replace(/^['"]|['"]$/g, '').trim()
  if (!t) {
...
async function restChunk (tickers, tok) {
  ...
  try {
    ...
    if (r.status === 401 || r.status === 403) {
      const e = new Error('EODHD rejected the API token')
      e.status = 502
      e.hint = 'check EODHD_API_TOKEN for wrapping quotes or stray whitespace, then redeploy'
      throw e
    }
...
async function restQuotes (tickers, tok) {
  ...
  for (const s of settled) {
    if (s.status === 'rejected') {
      failure ??= s.reason instanceof Error ? s.reason : new Error(String(s.reason))
      failures.push(s.reason?.message || String(s.reason))
      continue
    }
  ...
  return { quotes: out, failures, failure }
}
...
export async function liveQuotes (tickers) {
  ...
  if (!got && rest.failure) throw rest.failure
